...
def answer_one():
    ...
    energy['Country']=energy['Country'].replace({"Republic of Korea": "South Korea","United States of America": "United States","United Kingdom of Great Britain and Northern Ireland": "United Kingdom","China, Hong Kong Special Administrative Region": "Hong Kong"}).str.replace('\d+', '')
    ...
    years=['Country','2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015']
    GDPyears=GDP[years]
    
    ScimEn=pd.read_excel('scimagojr-3.xlsx')
    ScimEn15=ScimEn[ScimEn['Rank']<=15]
    
    df=pd.merge(ScimEn15,GDPyears, how='inner', left_on='Country', right_on='Country')
    df_all=pd.merge(df,energy, how='inner', left_on='Country', right_on='Country')
...
def answer_two():
    ...
    ScimEn=pd.read_excel('scimagojr-3.xlsx')
    df=pd.merge(ScimEn,GDP, how='inner', left_on='Country', right_on='Country')
    l_inner=pd.merge(df,energy, how='inner', left_on='Country', right_on='Country').shape[0]

    l_allunique = pd.concat([ScimEn['Country'],energy['Country'],GDP['Country']]).unique().shape[0]


    return l_allunique-l_inner
...
def answer_four():
    ...
    GDPchange=(Top15['2015'].iloc[5])-(Top15['2006'].iloc[5])
...
def answer_six():
    ...
    percent=Top15['% Renewable'].loc[str(country)]
    ans=(country,percent)
    return ans
...
def answer_seven():
    ...
    ratio=Top15['Citation Ratio'].loc[str(country)]
    ans=(country,ratio)
    return ans
...
def answer_eight():
    ...
    country=Top15.sort_values(by='Capita', ascending=False).index[2]
...
def answer_nine():
    ...
    Correlation=Top15['Citable docs per Capita'].astype(float).corr(Top15['Energy Supply per Capita'],method='pearson')
    return Correlation
answer_nine()
...
def answer_eleven():
    ...
    final = Top15.set_index('Continent').groupby(level = 0)['Population'].agg({'size':np.size, 'sum':np.sum, 'mean':np.mean, 'std':np.std})
    return final
